...
(function () {
    ...
    /*───────────────────── DATE PICKER SETUP ───────────────────*/
    /*  ——— revised only this block ——— */

    /* normalise any text date to ISO-8601 YYYY-MM-DD */
    const toISO = txt => {
        const d = new Date(txt.replace(/\u202f/g, ' ')); // handle NBSP
    ...
    const refreshBookedDates = () => {
        bookedDates = new Set(
            qsa('[ab-form="date-booked"]')
                .map(el => toISO(el.textContent.trim()))
                .filter(Boolean)
        );
    };
    /* initial build + keep live for CMS content loaded later */
    refreshBookedDates();
    new MutationObserver(refreshBookedDates).observe(document.body, { childList: true, subtree: true });

    const checkDate = () => {
        if (!datePicked) return;
        const chosen = toISO(datePicked.value.trim());
        if (warningUnavailable)
            warningUnavailable.style.display = (chosen && bookedDates.has(chosen)) ? 'block' : 'none';
        updatePriceEstimate();
    };
